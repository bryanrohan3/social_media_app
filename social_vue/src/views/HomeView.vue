...
export default {
  ...
  methods: {
    ...
    async logAuthToken() {
      // Retrieve auth token from Vuex store
    ...
    debouncedSearch: debounce(async function () {
      console.log("User Profile:", this.userProfile); // Log the userProfile object
      console.log("User Profile:", this.userProfile); // Log the userProfile object

      // Check if user profile exists and contains the authToken property
      if (!this.userProfile || !this.userProfile.token) {
        // If user is not logged in or token is not available, handle accordingly
        console.log("User is not logged in or token is not available");
        return;
      }

      console.log("Token available:", this.userProfile.token);
      ...
      try {
        const token = this.userProfile.token; // Retrieve the authToken
        const config = {
          headers: {
